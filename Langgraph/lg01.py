...
# 노드 함수 1: 검색 노드
def retrieval_node(state: SimpleState) -> dict:
    '''검색노드 : 질문을 기반으로 관련 문서를 검색'''
    question = state['question']
    print(f'검색노드 실행 : {question}')
    # 시뮬레이션 : 실제는 retrieval.invoke(question) 사용
    mock_documents = [
...
# 노드 함수 2: 생성 노드
def generation_node(state: SimpleState) -> dict:
    '''생성노드 : 검색된 문서를 기반으로 답변 생성'''
    documents = state['document']
    # 시뮬레이션 : 실제는 llm.invoke(prompt) 사용
    context = '\n'.join([doc.page_content for doc in documents])
    mock_answer = f'문서기반 답변 : {context[:50]}'
    return {'answer': mock_answer}

# 노드 실행 시뮬레이션
state: SimpleState = {'question':'LangGraph란', 'document':[], 'answer':''}
print(f'[초기상태] question:{state["question"]}, docs = {len(state["document"])}')

# step1 : 검색노드 실행
update1 = retrieval_node(state)
state.update(update1)

# step2 : 생성노드 실행
update2 = generation_node(state)
...
# 조건부 엣지를 위한 결정 함수
# Literal 타입힘트 반드시 이 값들중에서 하나를 반환해야 한다는 의미
